...
base_notebook = {
 "cells": [
  ...
  },
  {
   ...
   "source": [
    "# 5. Configuration\n",
    "CONCURRENCY = 8 # Increased to aggressively utilize GPU memory\n",
    "BATCH_CHUNK_SIZE = 64 # How many images to load into RAM at a time\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "errored = 0\n",
    "\n",
    "print(f\"Starting concurrent evaluation (Concurrency={CONCURRENCY}) to maximize GPU...\", flush=True)\n",
    "with samples_path.open(\"w\", encoding=\"utf-8\") as samples_handle:\n",
    "    with concurrent.futures.ThreadPoolExecutor(max_workers=CONCURRENCY) as executor:\n",
    "        for batch in batched(dataset_gen, BATCH_CHUNK_SIZE):\n",
    "            if LIMIT and completed >= LIMIT:\n",
    "                break\n",
    "            \n",
    "            futures = [executor.submit(evaluate_one, item, adapter) for item in batch]\n",
